...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    closest, current_closest = centroids[0], distance(location, centroids[0]) 
    for centroid in centroids:
        value = distance(location, centroid)
        if value < current_closest:
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        lst.append([centroid, restaurant])
...
def find_centroid(cluster):
    ...
    lat = [x for x,y in locations]
    lon = [y for x,y in locations]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature in feature_fns:
        pred, r_squared = find_predictor(user, reviewed, feature)
        r_squares.append(r_squared)
        preds.append(pred)
    lst = zip(preds, r_squares)
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    d = {restaurant_name(r): None for r in restaurants}
