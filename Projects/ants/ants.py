...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=0, max_range=0):
        ...
        i = 0
        while place != hive:
            if place.bees and min_range == 0 and max_range == 0:
                return random_or_none(place.bees)
            if place.bees and i in list(range(min_range, max_range+1)):
                return random_or_none(place.bees)
...
class LongThrower(ThrowerAnt):
    ...
    name = 'Long'
    food_cost = 2
    min_range, max_range = 5, MAX_RANGE
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    def nearest_bee(self, hive):
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        """Eats the bee and sets the time to digest to 3 turns"""
        # BEGIN Problem 6
...
class ScubaThrower(ThrowerAnt):
    """ Type of ant that can stay in the water"""
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            place = self.place
            while place.exit:  #This will return None if we hit the end of the tunnel
                place = place.exit #Moving back a place
                
                if place.ant: #Checking if there is an ant in the place
                    if place.ant.is_container and place.ant.contained_ant:
                        apply_buff(place.ant.contained_ant)
                    apply_buff(place.ant)
